...
feature 'shipping address for subscription' do
  ...
  let!(:subscription_product1) do
    create(:subscription_product, name: 'Subscription 1')
  end
  let!(:subscription_product2) do
    create(:subscription_product, name: 'Subscription 2')
  end
  ...
  context 'same as billing address' do
    ...
    context 'checkout delivery page' do
      scenario 'checking the values for subscription shipment' do
        page.find(:css, '#order_use_billing').set(true)
        click_button 'Save and Continue'
        expect(page).to have_content('Subscription product 1')
        expect(page).to have_content('Subscription product 2')
